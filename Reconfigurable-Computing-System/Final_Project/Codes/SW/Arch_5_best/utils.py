...
from tensorflow.keras.layers import Conv2D, ZeroPadding2D, MaxPooling2D, Dense, Flatten
...
def define_model() -> Sequential:
	...
	model.add(Conv2D(filters=conv_filter_num, kernel_size=conv_kernel_size, strides=stride_size,
				     activation="relu", padding="valid", kernel_initializer="he_uniform",
					 input_shape=(30, 30, 1), name="convolution_layer"))
	model.add(MaxPooling2D(pool_size, name="max_pooling_layer"))
	model.add(Flatten(name="flatten_layer"))
	model.add(Dense(10, activation="softmax", name="dense_layer1"))
...
def gen_conv_params(layer:Conv2D, label:str, kr:str, kc:str, f:str) -> str:
	...
	res += "fixed_point_t " + label + "_weights [" + f + "][" + kr + "][" \
			+ kc + "]\n\t= {\n"
	...
	res += "fixed_point_t " + label + "_biases [" + f + "] = { "
...
def gen_dense_params(layer:Dense,
	...
	res += '// ' + label.capitalize() + ' layer weights.\n'
	res += 'fixed_point_t ' + label + '_weights[' + size0 + '][' + size1 + ']\n\t = {\n'
	...
	res += '// ' + label.capitalize() + ' layer biases.\n'
	res += 'fixed_point_t ' + label + '_biases [' + size1 + '] = { '
	for i in range(b.shape[0]):
		res += str(float(b[i]))
		if (i != b.shape[0]-1):
			res += ', '
...
def save_param_on_files(model: Sequential) -> None:
	...
	with open('Headers/definitions.h', 'w') as f:
		...
		print("const int INT_BITS = {};" .format(INT_BITS), file=f)
		print("const int FRAC_BITS = {};" .format(FRAC_BITS), file=f)
		print("typedef ap_fixed<INT_BITS + FRAC_BITS, INT_BITS> fixed_point_t;", file=f)
		print('\n#define DIGITS 10\n\n#define IMG_ROWS '
